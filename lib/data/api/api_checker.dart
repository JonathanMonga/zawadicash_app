...
class ApiChecker {
  static void checkApi(Response response) {
      if (response.statusCode == 401) {
        if (Get.currentRoute != RouteHelper.loginScreen) {
          ...
          Get.offAllNamed(
            RouteHelper.getLoginRoute(
              countryCode: Get.find<AuthController>(
                  tag: getClassName<AuthController>()).getCustomerCountryCode(),
              phoneNumber: Get.find<AuthController>(
                  tag: getClassName<AuthController>()).getCustomerNumber(),
            ),
          );

          showCustomSnackBar(
              response.body['message'] ?? 'unauthorized'.tr, isIcon: true);
        }
      }
        ...
        showCustomSnackBar('you are using vpn', isVpn: true,
            duration: const Duration(minutes: 10));
      } else {
        if(response.body != null ){
          showCustomSnackBar(response.body['message'] ?? ErrorBody
              .fromJson(response.body)
              .errors!
              .first
              .message ?? '', isError: true);
        } else{
          showCustomSnackBar('Our server is down or some thing went wrong!',isVpn: true, isError: true, duration: const Duration(minutes: 10));
        }
      }
  }
